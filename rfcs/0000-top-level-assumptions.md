...
| `noCallClass` | Assume that classes are always instantiated with `new` and that the code never tries to call them as function. This lets Babel skipping the `this instanceof ThisClass` check (`_classCallCheck`). | `loose` in `transform-classes` | |
